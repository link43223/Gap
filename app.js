...
function openAdmin() {
    document.getElementById("dropdown").classList.remove("show");
    document.getElementById("adminKeyScreen").style.display = "block";
    document.getElementById("adminPanelMain").style.display = "none";
...
function verifyAdminKey() {
    ...
    fetch("http://localhost:3003/verify-key", {method:"POST",body:JSON.stringify({key:key}),headers:{"Content-Type":"application/json"}})
    ...
    }).catch(function(){ document.getElementById("adminKeyError").textContent = "无法连接服务器"; document.getElementById("adminKeyError").style.display = "block"; });
}
function loadAdminArticleList() {
    var keys=Object.keys(articles), topicMap={science:"科学科技",health:"健康",life:"生活",culture:"文化",nature:"自然",sports:"体育",gaming:"游戏"}, topics={};
    for(var k of keys){var t=k.split("-")[0];if(!topics[t])topics[t]=[];topics[t].push(k);}
    var html='<button class="topic-btn active" onclick="adminSelectTopic(\'all\',this)">全部</button>';
    for(var t in topics)html+='<button class="topic-btn" onclick="adminSelectTopic(\''+t+'\',this)">'+(topicMap[t]||t)+'</button>';
    document.getElementById("adminTopics").innerHTML=html; adminSelectTopic("all",document.querySelector("#adminTopics .topic-btn"));
}
function adminSelectTopic(topic,btn){
    document.querySelectorAll("#adminTopics .topic-btn").forEach(function(b){b.classList.remove("active");});
    if(btn)btn.classList.add("active");
    var keys=Object.keys(articles), f=topic==="all"?keys:keys.filter(function(k){return k.split("-")[0]===topic;}), html="";
    for(var k of f)html+='<div class="bookmark-article-item" onclick="adminEditArticle(\''+k+'\')" style="font-size:13px;"><span>'+articles[k].title.substring(0,45)+'…</span></div>';
    document.getElementById("adminArticleList").innerHTML=html; document.getElementById("adminEditor").style.display="none";
}
function adminEditArticle(key){
    var a=articles[key];
    document.getElementById("adminEditKey").value=key;
    document.getElementById("adminEditTitle").value=a.title||"";
    document.getElementById("adminEditTitleCn").value=a.titleCn||"";
    document.getElementById("adminEditSource").value=a.source||"";
    document.getElementById("adminEditText").value=a.text||"";
    document.getElementById("adminEditImage").value=a.image||"";
    document.getElementById("adminEditor").style.display="block";
    document.getElementById("adminSaveStatus").textContent="";
}
function adminSave(){
    var k=document.getElementById("adminEditKey").value;
    var d={key:document.getElementById("adminKeyInput").value,article_key:k,
        title:document.getElementById("adminEditTitle").value,
        titleCn:document.getElementById("adminEditTitleCn").value,
        source:document.getElementById("adminEditSource").value,
        text:document.getElementById("adminEditText").value};
    var img=document.getElementById("adminEditImage").value;
    if(img)d.image=img;
    var btn=document.querySelector("#adminEditor .action-btn");
    btn.disabled=true;btn.textContent="保存中...";
    fetch("http://localhost:3003/save",{method:"POST",body:JSON.stringify(d),headers:{"Content-Type":"application/json"}})
    ...
    .then(function(r){
        ...
        else{document.getElementById("adminSaveStatus").textContent="❌ "+r.error;}
